...
it("renders <Redirect> if no token", () => {
  const wrapper = shallow(<RequireAuth {...mockProps} />);
...
it("passes {to} prop to <Redirect> if no token", () => {
  ...
  const wrapper = shallow(<RequireAuth {...mockProps} to={to} />);
...
it("renders children if token is present", () => {
  const wrapper = shallow(
    <RequireAuth {...mockProps} token="test">
...
it("renders nothing if no children and token is present", () => {
  const wrapper = shallow(<RequireAuth {...mockProps} token="test" />);
